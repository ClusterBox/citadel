...
- **Pipeline steps** (only if `citadel.yml` declares a `pipeline:` block,
  which needs `ClusterBox/citadel/action@v0.4.0` or later — older versions
  ignore it and run the default pipeline):
  ...
  automatic rollback does — `ecs:DescribeServices`,
  `ecs:DescribeTaskDefinition` and `ecs:UpdateService` (plus
  `ecs:RegisterTaskDefinition` when the old revision was deregistered) for
...
variable holding that content before invoking `citadel deploy` (also with
`skip-secrets: true`), so a
